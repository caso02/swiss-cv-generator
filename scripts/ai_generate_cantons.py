    ...
    for attempt in range(max_retries):
        try:
            ...
            if is_valid:
                ...
                console.print(f"[yellow]⚠️  Validation failed (attempt {attempt + 1}/{max_retries}):[/yellow]")
            ...
            console.print(f"[red]❌ JSON parse error (attempt {attempt + 1}/{max_retries}): {e}[/red]")
            if attempt < max_retries - 1:
                console.print("[dim]Retrying...[/dim]")
                time.sleep(1)
                continue
            ...
            if attempt < max_retries - 1:
                console.print(f"[yellow]⚠️  Error (attempt {attempt + 1}/{max_retries}): {e}[/yellow]")
                console.print("[dim]Retrying...[/dim]")
                time.sleep(1)
                continue
            else:
...
def insert_cantons(collection, cantons: List[Dict[str, Any]]) -> int:
    ...
    for canton_doc in cantons:
        ...
        try:
            ...
            console.print(f"[red]❌ Error inserting {canton_doc.get('code')}: {e}[/red]")
            continue
...
def main():
    ...
    try:
        ...
        console.print(f"[green]✅ Inserted/updated {inserted_count} cantons[/green]")
        console.print()
        ...
        console.print(f"[green]✅ Total cantons in database: {total_count}[/green]")
        console.print()
